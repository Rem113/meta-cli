...
pub async fn add_simulator(sim: &Simulator, path: &String) -> Result<(), Error> {
    ...
    match find_image(&docker, &image_tag).await {
        Ok(None) => return Err(Error::ImageError(String::from("Simulator already exists"))),
        Ok(Some(_)) => (),
        Err(error) => return Err(Error::DockerError(error.to_string())),
    };
...
pub async fn remove_simulator(sim: &Simulator) -> Result<(), Error> {
    let docker = Docker::new();
...
pub async fn list_simulators() -> Result<(), Error> {
...
pub async fn run_simulator(sim: Simulator) -> Result<(), Error> {
    let docker = Docker::new();
    ...
    match image {
        Ok(image) => match image {
            ...
            None => return Err(Error::ImageError(String::from("Image does not exist"))),
        },
        Err(error) => return Err(Error::DockerError(error.to_string())),
    };
